...
이 때, 임의의 point $x \in S^{d}$에 대해 open hemisphere $H(x) := \{y \in S^{d} \mid \ip{x, y} > 0\}$을 정의하자. 이 때 $H(x)$가 color $i$로 칠해진 $f \in F$를 포함하면 $x \in A_{i}$가 되도록 $A_{i}$를 정의하자. 자연스럽게 $1 \le i \le d$에 대해 $A_{i}$는 open이고, $A_{d+1} = S^{d} \setminus (A_{1} \cup \cdots \cup A_{d})$는 closed가 된다.
