...
$$C \subset U \cap V$$가 $$U \cap V$$의 기저라고 하자. Basis Extension theorem에 의해 $$C$$를 포함하는 $$U$$의 기저 $$B$$, $$V$$의 기저 $$D$$가 존재한다.이 때 $$\left<C \cup D\right> = \left<C\right> + \left<D\right> = U + V$$이고, $$C \cap D = B$$이므로 $$C \cup D$$는 linearly independent하다. 따라서 $$\dim(U + V) = |C \cup D| = |C| + |D| - |C \cap D|$$ . $$\square$$
